...
object Refs extends ZIOAppDefault {
  ...
  // obtain a value
  val mol: ZIO[Any, Nothing, RuntimeFlags] = atomicMOL.flatMap { ref =>
    ref.get // returns a UIO[Int], thread-safe getter
  }

  // changing
  val setMol: ZIO[Any, Nothing, Unit] = atomicMOL.flatMap { ref =>
    ref.set(100) // UIO[Unit], thread-safe setter
  }

  // get + change in ONE atomic operation
  val gsMol: ZIO[Any, Nothing, RuntimeFlags] = atomicMOL.flatMap { ref =>
    ref.getAndSet(500)
  }
  ...
  val updatedMolWithValue: ZIO[Any, Nothing, RuntimeFlags] = atomicMOL.flatMap { ref =>
