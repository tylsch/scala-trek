  ...
  * */
  ...
  def demoSupportTicketMailbox(): Unit = {
    val userGuardian = Behaviors.setup[Unit] { ctx =>
      val actor = ctx.spawn(LoggerActor[Command](), "ticketLogger", MailboxSelector.fromConfig("support-ticket-mailbox"))

      actor ! Log("This is a log that is received first bug processed last")
      actor ! SupportTicket("[P1] this thing is broken")
      actor ! SupportTicket("[P0] fix now!")
      actor ! SupportTicket("[P3] this thing is broken again")

      /*
      * When a thread gets allocated, whatever is in the mailbox gets handled
      * */
  ...
  def demoControlAwareMailbox(): Unit = {
    val userGuardian = Behaviors.setup[Unit] { ctx =>
      val actor = ctx.spawn(LoggerActor[Command](), "controlAwareLogger", MailboxSelector.fromConfig("control-mailbox"))
